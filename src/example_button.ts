...
async function discoverCharacteristics(peripheral: noble.Peripheral) {
  const services = (await peripheral.discoverServicesAsync(SERVICE_UUIDS).catch(handleError)) as noble.Service[];
  ...
  const characteristics = tmpChara.sort(function (a: noble.Characteristic, b: noble.Characteristic) {
