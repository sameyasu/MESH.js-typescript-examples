...
import { Brightness } from '../mesh.js/packages/block/Brightness'
const block = new Brightness()
...
async function main() {
  ...
  noble.on('discover', async (peripheral) => {
    ...
    // Check peripheral
    if (!Brightness.isMESHblock(peripheral.advertisement.localName)) {
      return
    }
    ...
    // Notify when brightness updates
    const command = block.createSetmodeCommand(Brightness.NotifyMode.UPDATE_BRIGHTNESS)
    await characteristics[2].writeAsync(command2buf(command), false).catch(handleError)

    // Event handler
    block.onSensorEvent = (proximity: number, brightness: number, requestId: number) => {
      console.log(`proximity: ${proximity}, brightness: ${brightness}, requestId: ${requestId}`)
    }
  })
}
